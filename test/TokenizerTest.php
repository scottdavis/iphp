...
class TokenizerTest extends PHPUnit_Framework_TestCase {
    ...
    public function opData() {
        return array(array(T_FOREACH, 'foreach'), array(T_IF, 'if'), array(T_ELSE, 'else'), array(T_CLASS, 'class'));
    }
    /**
     * @dataProvider opData
     */
    ...
    public function syntaxCheckData() {
        return array(array('$test = "foo";', true), array('test', true), array('foreach(', false), array('foreach(range(0,10) as $r){ echo $r;}', true), array('class foo }', false), array(substr(file_get_contents(__FILE__), 5, 0), true));
    }
    /**
